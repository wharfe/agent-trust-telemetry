...
Claude Code がこのリポジトリで作業を開始するための文脈・決定済み仕様をまとめたファイル。
...
This repository is **Layer 2 — During** in the [Agent Trust Suite](https://github.com/wharfe/agent-trust-suite).
...
- **Install:** `pip install agent-trust-telemetry` (v0.1.0-alpha) or from source: `pip install -e ".[dev]"`
- **Input:** JSON messages or JSONL streams conforming to the Message Envelope Schema (see below)
- **Output:** Trust evaluation results with risk scores and violation classifications (see Output Contract below)
...
```json
{
  ...
  "execution_phase":   "string (extensible)",
  "session_id":        "string",
  "trace_id":          "string",
  "turn_index":        0,
  "provenance":        ["string"],
...
| ADR | 論点 | 決定 |
|---|---|---|
| ADR-001 | `execution_phase` の管理 | 推奨値レジストリ方式（extensible string）。`execution_phase_group` は MVP 未実装 |
